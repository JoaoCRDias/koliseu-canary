...
function confirmTaskModalWindow(player, storage)
	...
	local function confirmCallback(player, button, choice)
		...
		elseif taskOptions.uniqueTask == true and player:hasActiveTaskInCategory(storage) then
			sameCategoryModalWindow(player)
		elseif taskOptions.uniqueTask == true and player:getActiveTaskCount() >= taskOptions.maxSimultaneousTasks then
...
function sameCategoryModalWindow(player)
	local title = taskOptions.selectLanguage == 1 and task_pt_br.title or "Task System"
	local completedMessage = taskOptions.selectLanguage == 1 and task_pt_br.sameCategoryError or "You already have an active task in this category."
	local window = ModalWindow {
		title = title,
		message = completedMessage
	}
...
function sendTaskModalWindow(player)
	...
	local function confirmCallback(player, button, choice)
		...
		elseif not player:canStartCustomTask(temptasks[id]) then
			errorModalWindow(player)
		elseif taskOptions.uniqueTask == true and player:hasActiveTaskInCategory(temptasks[id]) then
			sameCategoryModalWindow(player)
		elseif taskOptions.uniqueTask == true and player:getActiveTaskCount() >= taskOptions.maxSimultaneousTasks then
			uniqueModalWindow(player)
		else
