-- Eclipse Sovereign - Death Event
-- Teleports all players out of the siege area and rewards participants

local eclipseSovereignDeath = CreatureEvent("EclipseSovereignDeath")

-- Exit position after defeating Eclipse Sovereign
local EXIT_POSITION = Position(825, 766, 7) -- Cosmic Siege entry room
local SIEGE_LEVEL = 1000

function eclipseSovereignDeath.onDeath(creature)
	...
	for _, player in ipairs(playersInZone) do
		...
		if participants[player:getId()] then
			...
			if inbox then
				local tokens = inbox:addItem(60535, 20)
				if tokens then
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You received 20 Siege Tokens!")
				end
			end
		end
...
eclipseSovereignDeath:register()
