...
function inServiceYalaharReward.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.uid == 3088 then
		if player:getStorageValue(Storage.InServiceOfYalahar.Reward) ~= 1 then
	...
	player:setStorageValue(Storage.InServiceOfYalahar.Reward, 1)
	return true
end

inServiceYalaharReward:uid(3088, 3089, 3090, 3091)
