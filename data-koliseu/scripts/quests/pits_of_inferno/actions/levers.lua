...
local function createStones()
	...
	Game.setStorageValue(Storage.ThePitsOfInferno.Levers, 0)
end
...
function pitsOfInfernoLevers.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local leverCount = math.max(0, Game.getStorageValue(Storage.ThePitsOfInferno.Levers))
	if item.uid > 2049 and item.uid < 2065 then
		...
		Game.setStorageValue(Storage.ThePitsOfInferno.Levers, number)
