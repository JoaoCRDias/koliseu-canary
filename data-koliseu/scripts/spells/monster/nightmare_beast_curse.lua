...
function spell.onCastSpell(creature, var)
	local hasCasted = Game.getStorageValue(Storage.U12_00.TheDreamCourts.DreamScarGlobal.LastBossCurse)
	...
	if hasCasted == 0 then
		...
		for _, k in pairs(players) do
			...
			if player then
				player:setStorageValue(Storage.U12_00.TheDreamCourts.DreamScar.LastBossCurse, -1)
			end
		end
		...
		newPlayer:setStorageValue(Storage.U12_00.TheDreamCourts.DreamScar.LastBossCurse, 1)
		newPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The beast laid a terrible curse on you!")

		Game.setStorageValue(Storage.U12_00.TheDreamCourts.DreamScarGlobal.LastBossCurse, 1)
	end
end
