local bugBounty = TalkAction("!bugbounty")
local usageMessage = "Usage: !bugbounty <playername>,<coins>"

function bugBounty.onSay(player, words, param)
	if param == "" then
	...
	if targetPlayer then
		targetPlayer:addTransferableCoins(coins)
		targetPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You received %d Tibia Coins from the bug bounty program.", coins))
	else
	...
	if GameStore and GameStore.insertHistory then
		GameStore.insertHistory(accountId, GameStore.HistoryTypes.HISTORY_TYPE_NONE, string.format("Bug bounty reward (%d coins)", coins), coins, GameStore.CoinType.Coin)
	end

	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("Added %d coins to %s (account %d) via bug bounty.", coins, normalizedName, accountId))
	logger.info("[BugBounty] {} awarded {} coins to {} (account {}).", player:getName(), coins, normalizedName, accountId)
	return true
end

bugBounty:separator(" ")
bugBounty:groupType("god")
bugBounty:register()
