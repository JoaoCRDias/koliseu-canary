...
GameStore.OfferTypes = {
	...
	OFFER_TYPE_TASK_ENHANCEMENT = 28,
	OFFER_TYPE_MW_SKIN = 29,
	OFFER_TYPE_MINING_BOOST = 30,
}
...
function convertType(type)
	local types = {
		...
		[GameStore.OfferTypes.OFFER_TYPE_MW_SKIN] = GameStore.ConverType.SHOW_ITEM,
	}
...
function onRecvbyte(player, msg, byte)
	...
	-- Note: exhaustion is already handled by the module delay system (canRunModule/setModuleDelay)
	-- configured in modules.xml. Using canDoExAction here caused issues because:
	-- 1) It shares cooldown with ALL game actions (use item, move item, etc.)
	-- 2) When exhausted, the early return did not consume msg bytes, potentially
	--    desynchronizing the protocol parser that runs after this module
...
function parseBuyStoreOffer(playerId, msg)
	...
	if
			...
			or (
				...
				and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_GEM_EXTENDER
				and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TASK_ENHANCEMENT
				and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_MW_SKIN
				and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_MINING_BOOST
	...
	local offerPrice = offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and GameStore.ExpBoostValues[math.max(1, player:getStorageValue(GameStore.Storages.expBoostCount))] or offer.price
	...
	if offerPrice > 0 and not player:canPayForOffer(offerPrice, offerCoinType) then
		logger.warn("[parseBuyStoreOffer] Coins insuficientes para '{}': tem {} coins, precisa {} (offer: {}, boostCount storage: {})",
			player:getName(), player:getTibiaCoins(), offerPrice, offer.name or "?",
			player:getStorageValue(GameStore.Storages.expBoostCount))
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_GEM_EXTENDER then
			GameStore.processGemExtenderPurchase(player, offer)
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_TASK_ENHANCEMENT then
			GameStore.processTaskEnhancementPurchase(player)
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_MW_SKIN then
			GameStore.processMWSkinPurchase(player, offer)
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_MINING_BOOST then
			GameStore.processMiningBoostPurchase(player)
	...
	if not pcallOk then
		...
		if not pcallError.code then
			...
			logger.warn("[parseBuyStoreOffer] - Purchase failed for player '{}', offer '{}' (id: {}, type: {}): {}", player:getName(), offer.name or "unknown", offer.id or "nil", offer.type or "nil", pcallError.message or "unknown error")
		end

		return queueSendStoreAlertToUser(alertMessage, 500, playerId)
	end

	if table.contains({ GameStore.OfferTypes.OFFER_TYPE_HOUSE, GameStore.OfferTypes.OFFER_TYPE_EXPBOOST, GameStore.OfferTypes.OFFER_TYPE_PREYBONUS, GameStore.OfferTypes.OFFER_TYPE_BLESSINGS, GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS, GameStore.OfferTypes.OFFER_TYPE_INSTANT_REWARD_ACCESS, GameStore.OfferTypes.OFFER_TYPE_GEM_EXTENDER, GameStore.OfferTypes.OFFER_TYPE_TASK_ENHANCEMENT, GameStore.OfferTypes.OFFER_TYPE_MW_SKIN, GameStore.OfferTypes.OFFER_TYPE_MINING_BOOST }, offer.type) then
	...
	if configure ~= GameStore.ConfigureOffers.SHOW_CONFIGURE then
		if not player:makeCoinTransaction(offer) then
			return queueSendStoreAlertToUser("Purchase transaction error. Please try again.", 350, playerId, GameStore.StoreErrors.STORE_ERROR_PURCHASE)
		end

		-- Mark one-per-account offers as purchased
		if offer.onePerAccount then
			local accountId = player:getAccountId()
			if accountId and accountId > 0 then
				kv.scoped("account"):scoped(tostring(accountId)):set("store-purchased-" .. offer.itemtype, true)
			end
		end
...
function Player.canBuyOffer(self, offer)
	...
	if disabled ~= 1 then
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_TASK_ENHANCEMENT then
			if self:getStorageValue(58100) >= 1 then
				disabled = 1
				disabledReason = "You already own the Task Enhancement."
			end
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_MINING_BOOST then
			if self:getStorageValue(58101) >= 1 then
				disabled = 1
				disabledReason = "You already own the Mining Boost."
			end
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_MW_SKIN then
			local kvScope = self:kv():scoped("mw-skins")
			if kvScope:get("skin_" .. offer.itemtype) then
				disabled = 1
				disabledReason = "You already own this Magic Wall skin."
			end
		end
	end

	-- Generic one-per-account check
	if disabled ~= 1 and offer.onePerAccount then
		local accountId = self:getAccountId()
		if accountId and accountId > 0 then
			local accountKV = kv.scoped("account"):scoped(tostring(accountId))
			if accountKV:get("store-purchased-" .. offer.itemtype) then
				disabled = 1
				disabledReason = "This offer can only be purchased once per account."
			end
		end
	end
		...
		if count > 0 then
			...
			for _, off in ipairs(offer.offers) do
				...
				if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
					local boostCount = player:getStorageValue(GameStore.Storages.expBoostCount)
					if boostCount < 1 then
						boostCount = 1
					end
					xpBoostPrice = GameStore.ExpBoostValues[boostCount]
				end
...
function sendShowStoreOffersOnOldProtocol(playerId, category)
	...
	for _, offer in ipairs(category.offers) do
		if count > 0 and offer.coinType == GameStore.CoinType.Coin then
			...
			local offerPrice = offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and GameStore.ExpBoostValues[math.max(1, player:getStorageValue(GameStore.Storages.expBoostCount))] or (newPrice or offer.price or 0xFFFF)
...
function sendStoreTransactionHistory(playerId, page, entriesPerPage)
	...
	for k, entry in ipairs(entries) do
		...
		msg:addByte(entry.mode <= 2 and entry.mode or 0) -- 0 = normal, 1 = gift, 2 = refund (3 = transfer, sent as 0 to client)
...
GameStore.insertHistory = function(accountId, mode, description, coinAmount, coinType, playerId)
	if playerId then
		return db.query(string.format("INSERT INTO `store_history`(`account_id`, `player_id`, `mode`, `description`, `coin_type`, `coin_amount`, `time`) VALUES (%s, %s, %s, %s, %s, %s, %s)", accountId, playerId, mode, db.escapeString(description), coinType, coinAmount, os.time()))
	end
...
function GameStore.processExpBoostPurchase(player)
	...
	if expBoostCount == -1 or expBoostCount == 0 then
		expBoostCount = 1
	end
	if expBoostCount > 5 then
		return error({ code = 1, message = "You can't buy more XP Boosts today." })
	end
end
...
function GameStore.processGemExtenderPurchase(player, offer)
	if not GemBag or type(GemBag.grantGemExtender) ~= "function" then
		return error({ code = 0, message = "Gem Bag system unavailable. Your purchase has been cancelled." })
	end

	if GemBag.playerHasExtender(player) then
		return error({ code = 0, message = "You already own the Gem Extender." })
	end

	if not GemBag.grantGemExtender(player) then
		return error({ code = 0, message = "Unable to deliver the Gem Extender. Please try again later." })
	end
end

function GameStore.processTaskEnhancementPurchase(player)
	if player:getStorageValue(58100) == 1 then
		return error({ code = 0, message = "You already own the Task Enhancement." })
	end
	player:setStorageValue(58100, 1)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
		"[Store] Task Enhancement activated! Your tasks will now auto-complete, auto-restart, grant +1000 HTP per completion, and your daily limit is increased to 4.")
end

function GameStore.processMiningBoostPurchase(player)
	if player:getStorageValue(58101) == 1 then
		return error({ code = 0, message = "You already own the Mining Boost." })
	end
	player:setStorageValue(58101, 1)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
		"[Store] Mining Boost activated! You now gain +1 mining try per tick and +5% success chance.")
end

function GameStore.processMWSkinPurchase(player, offer)
	local kvScope = player:kv():scoped("mw-skins")
	if kvScope:get("skin_" .. offer.itemtype) then
		return error({ code = 0, message = "You already own this Magic Wall skin." })
	end
	kvScope:set("skin_" .. offer.itemtype, true)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE,
		string.format("[Store] Magic Wall skin '%s' unlocked! Use !mw to select it.", offer.name))
end
...
function Player.removeCoinsBalance(self, coins)
	if self:canRemoveCoins(coins) then
		if not self:removeTibiaCoins(coins) then
			return false
		end
...
function Player.removeTransferableCoinsBalance(self, coins)
	if self:canRemoveTransferableCoins(coins) then
		if not self:removeTransferableCoins(coins) then
			return false
		end
		sendStoreBalanceUpdating(self:getId(), true)
		return true
...
function Player.makeCoinTransaction(self, offer, desc)
	...
	if op then
		if offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST then
			local currentCount = self:getStorageValue(GameStore.Storages.expBoostCount)
			if currentCount < 1 then
				currentCount = 1
			end
			self:setStorageValue(GameStore.Storages.expBoostCount, currentCount + 1)
		end
		GameStore.insertHistory(self:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, desc, offer.price * -1, offer.coinType, self:getGuid())
	end
