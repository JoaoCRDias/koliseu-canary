...
void Player::addBestiaryKill(const std::shared_ptr<MonsterType> &mType) {
	...
	// Winter Update 2025 - Bounty Talisman bestiary bonus (chance to double bestiary kills)
...
bool Player::onKilledMonster(const std::shared_ptr<Monster> &monster) {
	...
	if (!monster->getSoulPit()) {
		...
		// Winter Update 2025 - Task Board kill tracking
...
uint16_t Player::getSkillLevel(skills_t skill) const {
	...
	if (skill == SKILL_FIST && getVirtue() == VIRTUE_JUSTICE) {
		...
		const uint16_t bonusVirtueJustice = static_cast<uint16_t>(skillFist * (isSerene() ? 0.16 : 0.08));
...
void Player::sendItemInspection(uint16_t itemId, uint8_t itemCount, const std::shared_ptr<Item> &item, InspectObjectType inspectionType) const {
	if (client) {
...
void Player::sendWeeklyTaskData() {
	...
	// Ensure kill/delivery vectors have the exact expected count before sending.
	// Generates missing entries on the fly if the generation loop didn't produce enough.
